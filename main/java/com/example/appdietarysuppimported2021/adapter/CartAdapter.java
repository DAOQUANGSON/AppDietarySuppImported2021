package com.example.appdietarysuppimported2021.adapter;
...
import com.example.appdietarysuppimported2021.model.Product;
...
public class CartAdapter extends RecyclerView.Adapter<CartAdapter.CartViewHolder> {
    ...
    public void onBindViewHolder(@NonNull CartViewHolder holder, int position) {
        ...
        if (product.getSaleOff() > 0){
            holder.tvPrice.setText(formatter.format((product.getPrice() * ((100 - product.getSaleOff()) / 100) )) + " Đ");
            holder.tvPriceSale.setText(Html.fromHtml( "<del>"+formatter.format(product.getPrice())+" Đ</del>"+ " "  + String.valueOf(product.getSaleOff()).replace(".0"," %")));
        }else{
